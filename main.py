...
class ItemRows(MDBoxLayout):
    evt_time = StringProperty(datetime.now().strftime("%H:%M:%S:%p"))
    itemName = StringProperty('Inyass')

    theme_measure_1 = StringProperty('Cartoon')
    theme_measure_2 = StringProperty('Half')
    theme_measure_3 = StringProperty('Pieces')

    rollPrice = NumericProperty(10000)
    halfPrice = NumericProperty(5000)
    packetPrice = NumericProperty(500)
...
class TheGridLayer(MDGridLayout):
    ...
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        Clock.schedule_once(self.get_data, 3)  # default to 3

    def get_data(self, dt=None):
        for i in range(17):
            self.INDEX += 1
            Calculator.row = self.INDEX
            self.add_widget(ItemRows())  # home calc

            if self.abc:
                self.abc[0].ids.res_cont.text = "Contents: " + str(self.INDEX)
                self.abc[1].ids.content_txt.text = "Contents: " + str(self.INDEX)
...
class Container(MDScreenManager):
    ...
    def on_pre_enter(self, *args):
        Clock.schedule_once(self.direct_app_content_page, 3)  # default to 3
